...
#### `if-case` — fused conditional + pattern match
A control-flow construct that runs a branch only when a value matches a pattern, binding any captured variables into the body's scope. Replaces the old `is` check + cast dance and works as both a statement and a collection element inside widget trees.

```dart
// Statement form — type-safe replacement for `is` + cast
...
// With a guard clause
if (user case Admin(:final permissions) when permissions.canDelete) {
  deleteRecord();
}

// Collection-element form — declarative widget trees
Column(
  children: [
    if (result case Success(:final data))
      Text('Loaded ${data.length} items')
...
Reach for `if-case` when handling **one** shape of a value with a fall-through. Once you're handling three or more variants of a sealed type, prefer `switch` (statement or expression) so the compiler enforces exhaustiveness — `if-case` gives no such safety net.
...
Navigation is a cross-cutting concern that doesn't belong to any single release — Navigator 2.0 has been around since Flutter 1.22 (2020), and the ecosystem around it has matured steadily across the 3.10 → 3.41 window. If you're modernizing an old project, this is one of the bigger architectural decisions to revisit.

### The fundamental shift

**Navigator 1.0 is imperative. Navigator 2.0 is declarative.**

In 1.0, you *push* and *pop* routes — you tell the navigator what to do next. In 2.0, you give the navigator a list of pages that reflects your app state, and it figures out the transitions. Same shift Flutter made for widgets: describe the desired state, let the framework reconcile.

### Side-by-side

| Concern              | Navigator 1.0                          | Navigator 2.0                                |
|----------------------|----------------------------------------|----------------------------------------------|
| Style                | Imperative                             | Declarative                                  |
| Core API             | `Navigator.push` / `pop`               | `Navigator(pages: [...])` + `Router`         |
| Route type           | `Route<T>` / `MaterialPageRoute`       | `Page<T>` (immutable, e.g. `MaterialPage`)   |
| State source         | Lives inside the navigator stack       | Lives in your app state (Cubit/Bloc)         |
| Deep links           | Awkward — manual `onGenerateRoute`     | First-class via `RouteInformationParser`     |
| Web URL sync         | Limited / hacky                        | Built-in via `RouterDelegate`                |
| Browser back button  | Doesn't always match app state         | Properly synchronized                        |
| Boilerplate          | Minimal                                | Significant (without a package)              |
...
Fine for small apps, fine for in-flow modals and one-off screens. The state of "where am I" is implicit in the navigator's internal stack.

### Navigator 2.0 — declarative

You hand the navigator a `List<Page>` derived from app state. Change the state, the stack updates.

```dart
// package/lib/src/navigation/app_router.dart
class AppRouter extends StatelessWidget {
  const AppRouter({required this.state, super.key});
    ...
    return Navigator(
      pages: [
        ...
        if (state.selectedItemId != null)
          MaterialPage(child: DetailsScreen(id: state.selectedItemId!)),
        if (state.isEditing)
          const MaterialPage(child: EditScreen()),
      ],
      onDidRemovePage: (page) {
        // Sync state back when system pops (e.g. back button)
        context.read<NavCubit>().handlePop(page);
      },
    );
  }
}
...
No `push`, no `pop`. The stack *is* a function of state. Want to navigate? Mutate state.

### The full Navigator 2.0 stack

For deep-link / browser-URL parity, 2.0 introduces three pieces beyond `Navigator`:

- **`RouterDelegate`** — builds the `Navigator` from app state, listens for state changes
- **`RouteInformationParser`** — converts URL ↔ app state object
- **`BackButtonDispatcher`** — routes system back-button presses to the delegate

You wire them via `MaterialApp.router(...)` instead of `MaterialApp(...)`. This is where the boilerplate gets heavy — which is why almost nobody writes raw 2.0 anymore.

### What people actually use

Raw Navigator 2.0 is verbose. The community settled on packages built *on top* of it:

- **`go_router`** — official Flutter team package, declarative routes via path strings, handles deep links and web URLs cleanly. The default recommendation for new apps.
- **`auto_route`** — code-generation-based, more opinionated, strong for large apps with complex nested navigation.
- **`beamer`** — location-based, less common but well-designed.

These give you 2.0's benefits (deep links, URL sync, declarative stack) without writing a `RouterDelegate` by hand.

### Recent framework support

A few changes in the 3.10 → 3.41 window are worth flagging for navigation work:

- **Flutter 3.16** — Predictive back navigation (Android 14+) lands; routes need to opt in via `PopScope` (which replaced the deprecated `WillPopScope`).
- **Flutter 3.22** — `PopScope` API stabilized for predictive back. Migrate any lingering `WillPopScope` usages.
- **Flutter 3.27+** — `onPopInvokedWithResult` replaces `onPopInvoked` on `PopScope` for proper result propagation.
- **Flutter 3.29+** — `Navigator.onDidRemovePage` callback recommended over the older `onPopPage` for the declarative API.

### When to use which

Use **Navigator 1.0** for transient flows inside a screen — modals, full-screen dialogs, multi-step forms, image viewers. `showDialog`, `showModalBottomSheet`, and ad-hoc `push` calls are still idiomatic and not going anywhere.

Use **Navigator 2.0** (via `go_router`) for the app's primary navigation skeleton — anything that should reflect in the URL on web, anything reachable by deep link, anything that needs to survive process death and restore correctly. For a Cubit-driven architecture, declarative routing pairs naturally with state — a `NavigationCubit` emits state, the router rebuilds the stack, no imperative bridge between business logic and navigation.

The two coexist fine. `go_router` apps still call `context.pop()` for dialogs and use `Navigator.push` for genuinely local flows. The 1.0 vs 2.0 choice is about the *primary* navigation model, not an exclusive lock-in.

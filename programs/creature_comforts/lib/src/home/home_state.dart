// programs/creature_comforts/lib/src/home/home_state.dart
import 'package:creature_comforts_service/creature_comforts_service.dart'
    show CrittersStatus, LastFedFailure;
import 'package:equatable/equatable.dart';

/// State machine for [HomeCubit].
///
/// Three terminal shapes correspond to the three render modes the home
/// screen needs:
///
///  - [HomeInitializing] — first stream event has not yet arrived;
///    children show loading affordances (spinner for [LastFedDisplay],
///    a placeholder asset for `_PetImage`).
///  - [HomeFailure]      — stream emitted a [LastFedFailure]; children
///    render error affordances. Carries the failure so consumers can
///    surface its message.
///  - [HomeReady]        — a timestamp has been observed. Carries the
///    timestamp, the derived [CrittersStatus] (single source of truth
///    for the alive/dead rule), and a [tick] counter incremented by
///    the cubit every minute so widgets that render elapsed-time
...
final class HomeInitializing extends HomeState {
  const HomeInitializing();
}
...
/// it — matches the previous `_FailureBlock` behavior in
/// `LastFedDisplay`.
...
final class HomeReady extends HomeState {
  ...
  /// alive/dead rule across the home screen.

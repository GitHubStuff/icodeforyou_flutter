...
void main() {
  ...
  group('ServiceItemTimeout', () {
    ...
    test('appends "not ready" diagnostic when notReadyYet is populated', () {
      // Covers `_buildMessage`'s `notReadyYet.isNotEmpty` branch — the
      // adapter's `WaitingTimeOutException` path feeds this field so
      // diagnostic output can be reconstructed at the locator level.
      final error = ServiceItemTimeout(
        'database',
        const Duration(milliseconds: 500),
        notReadyYet: const ['cache', 'logger'],
      );
      expect(
        error.message,
        'Timeout on service "database" after 500ms; '
        'not ready: cache, logger',
      );
      expect(error.notReadyYet, const ['cache', 'logger']);
      expect(error.waitedBy, isEmpty);
    });

    test('appends "waited by" diagnostic when waitedBy is populated', () {
      // Covers `_buildMessage`'s `waitedBy.isNotEmpty` branch in isolation.
      final error = ServiceItemTimeout(
        'database',
        const Duration(milliseconds: 500),
        waitedBy: const {
      ...
      expect(
        error.message,
        'Timeout on service "database" after 500ms; '
        'waited by: {cache: [logger, db]}',
      );
      expect(error.notReadyYet, isEmpty);
      expect(error.waitedBy, const {
        'cache': ['logger', 'db'],
      });
    });

    test('appends both diagnostics when both fields are populated', () {
      // Covers the case where both conditional branches fire in
      // `_buildMessage`, producing the full diagnostic line.
      final error = ServiceItemTimeout(
        'database',
        const Duration(milliseconds: 500),
        notReadyYet: const ['cache'],
        waitedBy: const {
          'cache': ['db'],
        },
      );
  ...
  group('ServiceStartupFailed', () {
    test('message includes name but not the cause', () {
      // `.message` is the bare status line; the cause appears in
      // `toString()` on its own `Caused by:` line (Throwable-style
      // cause chain), not appended to the message itself.
      ...
      expect(error.message, '"database" failed to start');
    });
    ...
    test('toString() includes the message, cause, and stack trace', () {
      ...
      // Cause and message live on separate lines in the Throwable-style
      // layout: the message line identifies *what* failed; the
      // `Caused by:` line carries the underlying exception.
      expect(rendered, contains('ServiceStartupFailed:'));
      expect(rendered, contains('"database" failed to start'));
      expect(rendered, contains('Caused by: Exception: connection refused'));
      expect(rendered, contains('frame0'));
      expect(rendered, contains('frame1'));
      expect(rendered, contains('frame2'));
    });

    test('toString() format: message, then "Caused by: <cause>", then stack', () {
      ...
      expect(
        error.toString(),
        'ServiceStartupFailed: "database" failed to start\n'
        'Caused by: Exception: boom\n'
        'my-frame',
      );
    });
  });

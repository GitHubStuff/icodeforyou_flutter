// packages/analog_clock_widget/lib/src/clock_painter.dart
...
class _ClockPainter extends CustomPainter {
  ...
  void _drawModernTicks(Canvas canvas) {
    ...
    for (
      var i = 0;
      i < _ClockConstants.totalTicks;
      i += _ClockConstants.ticksPerHour
    ) {
      final angle = i * _ClockConstants.radiansPerTick;
      final tickStart = _calculateTickStart(angle);
  ...
  void _drawMinimalTicks(Canvas canvas) {
    ...
    for (
      var i = 0;
      i < _ClockConstants.totalTicks;
      i += _ClockConstants.ticksPerQuarter
    ) {
      final angle = i * _ClockConstants.radiansPerTick;
      final dotDistance =
          _radius -
          _tickConfig.longLength * _ClockConstants.minimalDotDistanceMultiplier;
      ...
      final dotRadius =
          _tickConfig.strokeWidth * _ClockConstants.minimalDotRadiusMultiplier;
  ...
  Offset _calculateNumberPosition(int tickIndex) {
    ...
    final angle =
        (tickIndex - _ClockConstants.topPositionTickOffset) *
        _ClockConstants.radiansPerTick;
    final numberRadius =
    ...
    switch (configuration.style.handStyle) {
      ...
      case HandStyle.modern:
        return baseStrokeWidth *
            multiplier *
            _ClockConstants.modernHandThicknessFactor;
      case HandStyle.sleek:
        return baseStrokeWidth *
            multiplier *
            _ClockConstants.sleekHandThicknessFactor;
    }
  }
  ...
  void _drawCenterDot(Canvas canvas, double baseStrokeWidth) {
    ...
    final dotRadius = switch (configuration.style.handStyle) {
      HandStyle.traditional =>
        baseStrokeWidth * _ClockConstants.traditionalDotMultiplier,
  ...
  double _calculateMinuteAngle() {
    final minuteSweep = _radius < _ClockConstants.minimumRadius
        ? 0.0
        : (dateTime.second / _ClockConstants.secondsPerMinute -
              _ClockConstants.minuteSweepOffset);

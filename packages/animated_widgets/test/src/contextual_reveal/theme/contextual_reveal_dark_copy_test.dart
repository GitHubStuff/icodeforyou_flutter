// packages/animated_widgets/test/src/contextual_reveal/theme/contextual_reveal_dark_copy_test.dart
...
void main() {
  // Override values chosen to differ from every dark default, so each
  // assertion proves the copy stores what it was given rather than
  // falling through to the base ContextualRevealDark value.
  ...
  group('_ContextualRevealDarkCopy', () {
    test('copyWith builds a copy exposing every overridden value', () {
      const base = ContextualRevealDark();
      ...
      expect(copy, isA<ContextualRevealDark>());

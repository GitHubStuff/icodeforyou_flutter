// ignore_for_file: always_use_package_imports, public_member_api_docs, comment_references
...
/// shown again for the lifetime of this [SplashFlow].
///
/// If any background task fails, [backgroundTaskFailedWidgetBuilder] is shown.
/// If the indeterminate phase exceeds [timeout], [timeoutWidget] is shown.
///
...
/// in [Center] or [SizedBox.expand]; [SplashFlow] owns that layout
/// contract.
class SplashFlow extends StatefulWidget {
  ...
  ///
  /// These futures must already be running when passed in; [SplashFlow]
  ...
  /// The widget shown when the indeterminate phase exceeds [timeout].
  /// Defaults to a plain `Time Out` text. Rendered full-screen and
  /// centered.
  final Widget? timeoutWidget;

  /// Builder for the widget shown when any background task fails. Receives
  /// the first error and its stack trace. Defaults to a plain text message.
  /// Rendered full-screen and centered.

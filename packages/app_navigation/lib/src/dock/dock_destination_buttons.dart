// packages/app_navigation/lib/src/dock/dock_destination_buttons.dart
...
/// The dock's destination buttons: one [DualNavButton] per member of
/// [visible], in order, followed — when [overflowed] is non-empty —
/// by a [DualOverflowButton] whose popover lists the [overflowed]
/// members as [DualPopoverTile]s.
///
...
/// row of [DualNavButton]s. When it is non-empty, the
...
/// widgets. Each member's [NavigableDestinationAbstract.iconData] and
/// [NavigableDestinationAbstract.caption] are wrapped in an [Icon] and [Text]
...
/// construction.
/// {@endtemplate}
class DockDestinationButtons<T extends NavigableDestinationAbstract>
  ...
  /// space between, and after the buttons. [MainAxisAlignment.spaceAround]
  ...
  Widget _dockButton(T destination) => DualNavButton(
  ...
  Widget _popoverTile(T destination) => DualPopoverTile<T>(
    value: destination,
    isSelected: destination == selected,
    icon: Icon(destination.iconData),

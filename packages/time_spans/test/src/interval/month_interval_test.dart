...
void main() {
  group('MonthInterval', () {
    group('microsecondMonths', () {
      ...
      test('decrements internal months if _addMonths overshoots the later date', () {
        ...
        final expected = DateTime(2024, 2, 29).difference(start).inMicroseconds;
    ...
    group('completedMonths', () {
      test('returns exact month count when asOf day is exact or later', () {
        expect(MonthInterval.completedMonths(DateTime(2024, 1, 15), DateTime(2024, 2, 15)), 1);
        expect(MonthInterval.completedMonths(DateTime(2024, 1, 15), DateTime(2024, 2, 16)), 1);
      });

      test('decrements month count if asOf day is earlier than the clamped start day', () {
        expect(MonthInterval.completedMonths(DateTime(2024, 1, 15), DateTime(2024, 2, 14)), 0);
      });

      test('handles clamped days correctly in a leap year (e.g. Jan 31 to Feb 29)', () {
        expect(MonthInterval.completedMonths(DateTime(2024, 1, 31), DateTime(2024, 2, 29)), 1);
      });

      test('handles clamped days correctly in a common year (e.g. Jan 31 to Feb 28)', () {
        expect(MonthInterval.completedMonths(DateTime(2023, 1, 31), DateTime(2023, 2, 28)), 1);
        expect(MonthInterval.completedMonths(DateTime(2023, 1, 31), DateTime(2023, 2, 27)), 0);
      });

      test('returns mathematically expected negative values for backward spans', () {
        // These assertions document the function's specific directional math 
        // when traversing backwards mathematically.
        expect(MonthInterval.completedMonths(DateTime(2024, 2, 15), DateTime(2024, 1, 16)), -1);
        expect(MonthInterval.completedMonths(DateTime(2024, 2, 15), DateTime(2024, 1, 14)), -2);
      });
    });

    group('completedYears', () {
      test('returns whole years accurately using completedMonths division', () {
        expect(MonthInterval.completedYears(DateTime(2000, 1, 1), DateTime(2002, 1, 1)), 2);
        expect(MonthInterval.completedYears(DateTime(2000, 1, 1), DateTime(2001, 12, 31)), 1);
        expect(MonthInterval.completedYears(DateTime(2000, 1, 1), DateTime(1998, 1, 1)), -2);
      });
    });
    ...
    group('monthCounter', () {
      test('returns 0 when start and finish are the exact same moment', () {
        ...
        expect(
          MonthInterval.monthCounter(start: t, finish: t, monthPolicy: MonthPolicy.clamp), 
          0,
        );
      });
      ...
      test('returns negative months when start is before finish', () {
        ...
        final finish = DateTime(2024, 2, 15);
        expect(
          MonthInterval.monthCounter(start: start, finish: finish, monthPolicy: MonthPolicy.clamp), 
          -1,
        );
      });

      test('returns positive months when start is after finish (recurses and uses absolute value)', () {
        final start = DateTime(2024, 2, 15);
        final finish = DateTime(2024, 1, 15);
        expect(
          MonthInterval.monthCounter(start: start, finish: finish, monthPolicy: MonthPolicy.clamp), 
      ...
      test('handles overshoot decrements correctly with MonthPolicy.overflow', () {
        ...
        expect(
          MonthInterval.monthCounter(start: start, finish: finish, monthPolicy: MonthPolicy.overflow), 
          0,
        );
      });
    ...
    group('averageDaysPerMonth', () {
      test('returns the correct highly-precise average for each duration tier', () {
        ...
        expect(MonthInterval.averageDaysPerMonth(1201), Span.kGregorianCycleAvg);
        expect(MonthInterval.averageDaysPerMonth(5000), Span.kGregorianCycleAvg);
      });
    });
  });
}
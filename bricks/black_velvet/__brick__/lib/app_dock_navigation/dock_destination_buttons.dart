// programs/{{name.snakeCase()}}/lib/app_dock_navigation/dock_destination_buttons.dart

import 'package:flutter/material.dart';
import 'package:rail_navigation/rail_navigation.dart'
    show RailButton, RailOverflowButton, RailPopoverTile;

import 'dock_destination_enum.dart' show DockDestinationEnum;
...
/// uses the same size so the dock's geometry stays uniform whether or
/// not it appears. A visible destination escapes this footprint only
/// through an explicit [DockDestinationButtons.sizeOverrides] entry.
const Size _kButtonSize = Size(80, 60);

/// {@template dock_destination_buttons}
/// The dock's destination buttons: one [RailButton] per member of
...
///
/// The caller owns the partition. Which destinations are dock
...
/// guesses nothing. When [overflowed] is empty, the dock is exactly a
/// row of [RailButton]s. When it is non-empty, the
/// overflow button always occupies the last slot, after every visible
/// button.
///
/// The caller also owns distribution. The dock fills its width and
/// spreads the buttons per [mainAxisAlignment]; the default
/// [MainAxisAlignment.spaceEvenly] gives the standard
...
/// widgets. Each member's [DockDestinationEnum.iconData] and
/// [DockDestinationEnum.caption] are wrapped in an [Icon] and [Text]
/// here, whether the member renders as a dock button or a popover
/// tile — the dock widgets render content as provided, so styling is
/// applied at this site.
///
/// Selection behaves as a radio group owned by the parent: [selected]
/// names the current destination, and every path — dock button tap or
...
/// construction.
/// {@endtemplate}
class DockDestinationButtons extends StatelessWidget {
  /// {@macro dock_destination_buttons}
  const DockDestinationButtons({
    required this.visible,
    required this.selected,
    required this.onSelect,
    super.key,
    this.overflowed = const <DockDestinationEnum>[],
    this.sizeOverrides = const <DockDestinationEnum, Size>{},
    this.mainAxisAlignment = MainAxisAlignment.spaceEvenly,
  });

  /// The destinations rendered as dock buttons, in this order.
  ///
  /// The caller decides membership and order; typically the app's
  /// primary destinations. Must not be empty.
  final List<DockDestinationEnum> visible;
  ...
  /// When non-empty, the overflow button is always the dock's last
  /// slot, after every [visible] button. Must not share a member with
  /// [visible].
  final List<DockDestinationEnum> overflowed;

  /// Per-destination size overrides for visible dock buttons.
  ///
  ...
  final DockDestinationEnum selected;

  /// Called with the chosen destination's enum member, whether it was
  /// tapped as a dock button or picked from the overflow popover.
  ///
  /// Dock button taps report every tap, including taps on the
  /// already-selected destination; whether that is a no-op is the
  /// parent's call. Dismissing the popover without a pick reports
  /// nothing.
  final ValueChanged<DockDestinationEnum> onSelect;

  /// How the buttons are distributed along the dock's width.
  ///
  /// The dock fills its width, so distribution alignments have room
  /// to work with. Defaults to [MainAxisAlignment.spaceEvenly], the
  /// standard bottom-navigation distribution: equal space before,
  ...
  /// The dock button for [destination], sized by its [sizeOverrides]
  /// entry when one exists and [_kButtonSize] otherwise.
  Widget _dockButton(DockDestinationEnum destination) => RailButton(
  ...
  Widget _popoverTile(DockDestinationEnum destination) =>
      RailPopoverTile<DockDestinationEnum>(
        value: destination,
        isSelected: destination == selected,
        icon: Icon(destination.iconData),
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      ...
      children: [
        for (final destination in visible) _dockButton(destination),
        if (overflowed.isNotEmpty)
          RailOverflowButton<DockDestinationEnum>(
            size: _kButtonSize,
